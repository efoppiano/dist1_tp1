...
class BasicPacket(ABC):
    def encode(self) -> bytes:
        ...
        elements_base64 = [base64.b64encode(element) for element in elements_bytes]
        return b",".join(elements_base64)

    @classmethod
    def decode(cls, data: Union[bytes, str]) -> "BasicPacket":
        class_fields = cls.__dict__["__dataclass_fields__"]
        if isinstance(data, str):
            elements_base64 = data.split(",")
        else:
            elements_base64 = data.split(b",")
        elements_bytes = [base64.b64decode(element) for element in elements_base64]
